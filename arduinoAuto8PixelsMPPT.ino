...
void perturbAndObserve() {
    ...
    setVoltage(&allDAC[pixelID],  Vset, pixelID);
    getINA129(&allINA219[pixelID], pixelID);
    avgPowerCalced = abs(loadvoltage * current_mA_Flipped);
    zero();

    while(millis()/1000 < 120) {
        // Vset + deltaV ------------------------------------------
        VsetUp = Vset + voltage_Step__Size_PnO;
        setVoltage(&allDAC[pixelID],  VsetUp, pixelID);
        getINA129(&allINA219[pixelID], pixelID);
        avgPowerCalcedUp = abs(loadvoltage * current_mA_Flipped);
        zero();
        ...
        setVoltage(&allDAC[pixelID],  VsetUp, pixelID);
        getINA129(&allINA219[pixelID], pixelID);
        avgPowerCalcedDown = abs(loadvoltage * current_mA_Flipped);
        zero();


        // FINISH CODING BELOW THIS LINE------------------
...
void scan(String dir) {
    ...
    Serial.print("started scan with step rate: "); Serial.println(delayTimeMS);
