...
void pno_vset_helper(int DEVICE, int direction) {
    ...
    for (int PIXEL = 0; PIXEL < 8; PIXEL++) {
        ...
        if (direction == 0) {
            if (MAXVOLTAGEPNO < Vset[DEVICE][PIXEL]) Vset[DEVICE][PIXEL] = MAXVOLTAGEPNO;
            Vset_val = Vset[DEVICE][PIXEL];
        } else if (direction > 0) {
            Vset_val = Vset[DEVICE][PIXEL] + pno_voltage_step_size;
        } else if (direction < 0) {
            Vset_val = Vset[DEVICE][PIXEL] - pno_voltage_step_size;
        }
        set_voltage(all_dac[PIXEL+8*DEVICE], PIXEL, TCAADR_DAC[DEVICE], Vset_val);
    }
