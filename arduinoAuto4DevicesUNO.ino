...
Adafruit_INA219 *allINA[4][8] =  {{&ina219_0, &ina219_1, &ina219_2, &ina219_3, &ina219_4, &ina219_5, &ina219_6, &ina219_7},
...
Adafruit_MCP4725 *allDAC [4][8] = {{&dac_0, &dac_1, &dac_2, &dac_3, &dac_4, &dac_5, &dac_6, &dac_7},
...
uint8_t TCAADR_INA[] = {0x70, 0x72, 0x74, 0x76};
uint8_t TCAADR_DAC[] = {0x71, 0x73, 0x75, 0x77};
...
void setup(void) {
    ...
    for (int DEVICE = 0; DEVICE < 4; DEVICE++) {
        for (int PIXEL = 0; PIXEL < 8; PIXEL++) {
            SetupSensors(allINA[DEVICE][PIXEL], allDAC[DEVICE][PIXEL], TCAADR_INA[DEVICE], TCAADR_DAC[DEVICE], PIXEL, DEVICE);
        }
    }
...
void perturbAndObserve() {
    ...
    Serial.println(measurement_Time/60/60/24);
        ...
        */
        for (int DEVICE = 0; DEVICE < 4; DEVICE++) {
            for (int PIXEL = 0; PIXEL < 8; PIXEL++) {
                ...
                // Vset --------------------------------------------------
                setVoltage(allDAC[DEVICE][PIXEL], PIXEL, TCAADR_DAC[DEVICE], Vset[DEVICE][PIXEL]);
                delay(measurement_Delay_PnO);
                for (int i = 0; i < measurements_Per_Step_PnO; ++i) {
                    // getINA129(&allINA219[ID], ID);
                    getVolCurr(allINA[DEVICE][PIXEL], TCAADR_INA[DEVICE], PIXEL);
...
void scan(String dir) {
    ...
    for (int DEVICE = 0; DEVICE < 4; DEVICE++) {
        for (int PIXEL = 0; PIXEL < 8; PIXEL++) {
            setVoltage(allDAC[DEVICE][PIXEL], PIXEL, TCAADR_DAC[DEVICE], VOLTAGE_SET_VAL);
        }
    }
...
void SetupSensors(Adafruit_INA219 *ina219,
    ...
    // for (int i = 0; i < 4; i++) {
    //     for (int ID = 0; ID < 8; ID++) {
        TCA9548Access(PIXEL, dac_bus_ID);
        if (!dac->begin())
        {
        ...
        TCA9548Access(PIXEL, ina_bus_ID);
        if (!ina219->begin())
        {
            Serial.print("Device_");
            Serial.print(DEVICE);
        ...
        ina219->setCalibration_16V_400mA();
            // SetupSensors(allINA[i][j], allDAC[i][j], j, TCAADR_INA[i], TCAADR_DAC[i]);
        // }
    // }
...
void TCA9548Access(uint8_t ID, uint8_t bus) {
...
void zero() {
    for (int DEVICE = 0; DEVICE < 4; DEVICE++) {
        for (int PIXEL = 0; PIXEL < 8; PIXEL++) {
            setVoltage(allDAC[DEVICE][PIXEL], PIXEL, TCAADR_DAC[DEVICE], 0);
            delay(30);
        }
    }
...
void getVolCurr(Adafruit_INA219 *ina219, uint8_t bus_ID, uint8_t PIXEL) {
    TCA9548Access(bus_ID, PIXEL);
